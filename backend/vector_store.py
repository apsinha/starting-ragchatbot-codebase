...
class VectorStore:
    ...
    def get_course_outline(self, course_name: str) -> Optional[Dict[str, Any]]:
        """Get the complete outline of a course by name (fuzzy matched).

        Returns dict with title, course_link, and lessons list, or None if not found.
        """
        import json

        # Resolve the course name via vector similarity
        course_title = self._resolve_course_name(course_name)
        if not course_title:
            return None
